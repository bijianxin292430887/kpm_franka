...
class IDM_Latent_Policy(nn.Module):
    ...
    def __init__(self,
                 ...
                 device: str = 'cuda'):
        super(IDM_Latent_Policy, self).__init__()
        ...
        self.latent_policy = DiffusionConditionalUnet1D(input_dim=input_dim,
                                                        global_cond_dim=OBS_HORIZON * (self.embedding_dim+input_dim)).to(device)
    ...
    def forward(self, obs, act):
        ...
        past_act_cond = act_cond.flatten(start_dim=1)
        past_cond = torch.cat((past_obs_cond,past_act_cond),dim=-1)

        # predict the noise residual
        noise_pred = self.latent_policy(
            noisy_z, timesteps, global_cond=past_cond)
...
class IDM_Latent_Policy_Agent(BaseAgent):
    def __init__(
            self,
            model: DictConfig,
        ...
        for num_epoch in tqdm(range(self.epoch)):
            ...
            if not (num_epoch + 1) % self.eval_every_n_epochs:
                ...
                for data in self.test_dataloader:
                    ...
                    act = data_normalize(data['spoon_poses'], self.norm_param, 'poses').cuda()
    ...
    def evaluate(self, state, action: Optional[torch.Tensor] = None, goal: Optional[torch.Tensor] = None):
        ...
        pred_act_future = self.model.sample(past_cond,act_future)
    ...
    @torch.no_grad()
    def predict(self, state, action: Optional[torch.Tensor] = None, if_vision=False,k_step=12) -> torch.Tensor:
        """
        ...
        obs_cond = state[:, 0:OBS_HORIZON]
        act_cond = action[:, 0:OBS_HORIZON]
        B,_,D = act_cond.shape
        dummy_act_future = torch.zeros((B,k_step,D)).to(self.device)
        ...
        n_pred_act_future = self.model.sample(past_cond, dummy_act_future)
